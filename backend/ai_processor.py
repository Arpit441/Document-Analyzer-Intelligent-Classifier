...
#     """
#     Parses AI response with fallback mechanisms.
...
#         logging.warning("No delimiter found, attempting JSON array parsing...")
#         try:
#             parsed_array = json.loads(response_text)
#             if isinstance(parsed_array, list):
#                 json_strings = [json.dumps(obj) for obj in parsed_array]
#                 logging.info(f"Successfully parsed as JSON array with {len(json_strings)} items.")
#             else:
#                 logging.error("Response is valid JSON but not an array")
#                 json_strings = [response_text]
#         except json.JSONDecodeError:
#             logging.error("Response is not valid JSON, treating as single chunk")
#             json_strings = [response_text]
...
#             logging.warning(f"Skipping malformed chunk: {json_str[:50]}...")
#             continue
...
#                 }
#                 logging.info(
#                     f"📧 Email forwarding analysis: is_forwarding={is_forwarding}, "
#                     f"type={forwarding_doc_type}, confidence={forwarding_confidence}"
#                 )
        
#         except json.JSONDecodeError as e:
#             logging.warning(f"Could not parse JSON chunk: {e}. Chunk: {json_str[:50]}...")
#             continue
...
# #         # Safely remove the outermost braces if the content is still delimited.
...
# #         logging.info(f"AI response contained {len(json_strings)} JSON chunks (delimiter method).")
# #     else:
# #         # Fallback: Try to parse as JSON array (omitted for brevity, keeping only the delimiter logic for this fix)
# #         json_strings = [response_text] # Treat as a single chunk if no delimiter found
...
# #         # 3. Aggressive character cleanup within the chunk itself
# #         # This removes control characters and extra quotes/braces that might be breaking JSON.
# #         if json_str.startswith('{') and json_str.endswith('}'):
# #              # Safely remove a single set of wrapping non-JSON chars like extra curly braces or quotes
# #             json_str = json_str.strip('\"\'{}').strip() 
# #             # Now ensure it begins and ends with proper JSON braces
...
# #         # Final check if it looks like a JSON object before attempting to load
# #         if not json_str.startswith('{') or not json_str.endswith('}'):
# #             logging.warning(f"Skipping malformed chunk: {json_str[:50]}...")
# #             continue
        
# #         try:
# #             # Re-encode quotes for resilience if AI incorrectly used single quotes (a common LLM JSON failure)
# #             # NOTE: This is a hack, but necessary for resilient parsing of LLM output
...
# #                         nested_doc['classification'] = value
# #                     else:
# #                         # Attempt to parse confidence safely, defaulting to 9 if value is not None
# #                         confidence = 10 if value is None else 9
# #                         # This part handles the case where the AI might return an unparsed confidence score
# #                         try:
# #                             if key.endswith('_confidence') and isinstance(value, str):
...
#         # Debug: Log each raw chunk
#         for i, chunk in enumerate(json_strings):
#             chunk_preview = chunk.strip()[:100]
#             logging.debug(f"   Raw chunk {i}: {chunk_preview}{'...' if len(chunk.strip()) > 100 else ''}")
#     else:
#         json_strings = [response_text]
...
#         # === COMPREHENSIVE JSON CLEANING PIPELINE ===
        
#         # Step 1: Remove any wrapping quotes that might be around the entire JSON
#         if (json_str.startswith('"') and json_str.endswith('"')) or \
#            (json_str.startswith("'") and json_str.endswith("'")):
#             json_str = json_str[1:-1]
#             logging.warning(f"Chunk {idx}: Removed wrapping quotes")
        
#         # Step 2: Handle multiple consecutive braces at start and end
#         leading_braces = 0
#         trailing_braces = 0
...
#         # If we have multiple leading braces, keep only one
#         if leading_braces > 1:
#             json_str = '{' + json_str[leading_braces:]
#             logging.warning(f"Chunk {idx}: Had {leading_braces} leading braces, reduced to 1")
        
#         # If we have multiple trailing braces, keep only one
#         if trailing_braces > 1:
#             json_str = json_str[:-trailing_braces] + '}'
#             logging.warning(f"Chunk {idx}: Had {trailing_braces} trailing braces, reduced to 1")
        
#         # Step 3: Handle missing opening brace
#         if not json_str.startswith('{') and ':' in json_str:
#             logging.warning(f"Chunk {idx}: Missing opening brace, adding it")
#             json_str = '{' + json_str
        
#         # Step 4: Handle missing closing brace by counting
...
#                 logging.warning(f"Chunk {idx}: Added {missing} missing closing brace(s)")
#             elif close_count > open_count:
#                 missing = close_count - open_count
#                 json_str = ('{' * missing) + json_str
#                 logging.warning(f"Chunk {idx}: Added {missing} missing opening brace(s)")
        
#         # Step 5: Replace single quotes with double quotes (invalid JSON)
#         # Only if there are no double quotes present (avoid breaking strings that contain quotes)
#         if '"' not in json_str and "'" in json_str:
#             json_str = json_str.replace("'", '"')
#             logging.warning(f"Chunk {idx}: Replaced single quotes with double quotes")
        
#         # Step 6: Final validation - must start with { and end with }
#         if not json_str.startswith('{') or not json_str.endswith('}'):
#             logging.error(f"❌ Chunk {idx} still malformed after cleanup: {json_str[:100]}...")
#             # Don't skip yet - try regex extraction below
...
#                 }
#                 logging.info(
#                     f"✅ Successfully parsed forwarding analysis chunk {idx}: is_forwarding={is_forwarding}, "
#                     f"type={forwarding_doc_type}, confidence={forwarding_confidence}"
#                 )
#             else:
#                 logging.warning(f"⚠️ Chunk {idx} parsed but doesn't contain expected keys: {list(parsed_flat.keys())}")
        
#         except json.JSONDecodeError as e:
#             logging.error(f"❌ JSON parsing failed for chunk {idx}: {e}")
#             logging.error(f"   Original: {original_str[:150]}...")
#             logging.error(f"   Cleaned:  {json_str[:150]}...")
#             parsing_successful = False
#         except Exception as e:
...
#             # This ensures we don't lose critical document types like EXEMPTION
#             try:
#                 # Use regex to extract key fields even from malformed JSON
...
#                     # Try to extract all other fields - handle both null and string values
#                     fields_to_extract = ['loan_id', 'client', 'property_address', 'parcel_number', 
#                                         'tax_year', 'base_tax_amount', 'total_tax_paid', 'date_paid', 'check_number']
                    
#                     for field in fields_to_extract:
#                         # Match both "field": "value" and "field": null patterns
#                         field_match = re.search(rf'"{field}"\s*:\s*"([^"]*)"', original_str)
#                         null_match = re.search(rf'"{field}"\s*:\s*null', original_str)
...
#                         else:
#                             # Field not found in the malformed JSON
...
#                     logging.warning(f"   Extracted fields with values: {[k for k, v in minimal_doc.items() if k != 'classification' and v.get('value')]}")
#                 else:
#                     logging.error(f"❌ Could not extract classification from chunk {idx}")
#             except Exception as regex_error:
#                 logging.error(f"❌ Regex extraction also failed for chunk {idx}: {regex_error}")
